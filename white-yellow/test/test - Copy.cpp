...
string BuildJoinedName(const vector<string>& cnames) {
    if (cnames.empty()) {
        return "";
    }
    vector<string> names = cnames;
...
class Person {
public:
    Person(const string& fname, const string& lname, int bdate) {
        birthdate = bdate;
        first_names[bdate] = fname;
        last_names[bdate] = lname;
    }
